...
import { SET_IS_AUTHENTICATING, LOGIN_FAILURE, LOGIN_SUCCESS, REGISTER_FAILURE, REGISTER_SUCCESS, SET_TOKEN, SET_IS_REGISTER_SUCCESS } from "./types";
...
export const setIsRegisterSuccess = (bool) => ({
  type: SET_IS_REGISTER_SUCCESS,
  payload: bool,
})
...
export const register = (email: string, password: string) => async (dispatch) => {
  try {
    ...
    dispatch({ type: REGISTER_SUCCESS, payload: token, isRegisterSuccess: true });
  } catch (error) {
